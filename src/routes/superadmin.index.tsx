              ...
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    >
                      {pieData.map((entry) => (
                        <Cell key={entry.plan} fill={PLAN_COLORS[entry.plan] ?? "#8A8A8A"} />
                      ))}
                    </Pie>
                ...
                {pieData.map((entry) => (
                  <span key={entry.plan} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                    <span
                      ...
                      style={{ backgroundColor: PLAN_COLORS[entry.plan] ?? "#8A8A8A" }}
                    />
      ...
      <div className={`${softCard} mt-6 overflow-hidden`}>
        <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-4 sm:px-5">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1 text-xs font-semibold text-[#28396C] hover:underline"
          >
            {st.viewAllCenters}
