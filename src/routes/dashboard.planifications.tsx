...
const PLANS = mirrorPlanificationEntries;
...
const toneBlock: Record<MirrorPlanTone, string> = {
...
function earliestPlanMonday(): Date {
  const min = PLANS.reduce(
    (acc, p) => (parseISO(p.date) < parseISO(acc) ? p.date : acc),
    PLANS[0].date,
  );
...
function planBlockStyle(plan: (typeof PLANS)[number]): { top: number; height: number } | null {
...
function PlanificationsPage() {
  ...
  const monthPlans = useMemo(() => {
    ...
    return PLANS.filter((p) => isSameMonth(parseISO(p.date), ref)).sort((a, b) => {
      const da = parseISO(a.date).getTime() - parseISO(b.date).getTime();
      if (da !== 0) return da;
      return a.time.localeCompare(b.time);
    });
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[640px]">
              ...
              >
                ...
                {weekDays.map((day) => {
                  const dayPlans = PLANS.filter((p) => isSameDay(parseISO(p.date), day));
                  return (
                    <div
                      key={format(day, "yyyy-MM-dd")}
                      className={cn(
