...
update public.clients
  ...
  where payment_status = 'en_attente'
    and (
      select coalesce(sum(amount), 0) from public.payments where client_id = id
    ) = 0;

alter table public.clients
  add constraint clients_payment_status_check
    check (payment_status in ('paye', 'en_attente', 'retard', 'impaye'));

alter table public.clients
  alter column payment_status set default 'impaye';
  ...
  where status = 'en_attente'
    and (
      ...
      where client_id = invoices.client_id
        and to_char(date, 'YYYY-MM') = invoices.period
    ) = 0;

alter table public.invoices
  add constraint invoices_status_check
    check (status in ('paye', 'en_attente', 'retard', 'impaye'));

alter table public.invoices
  alter column status set default 'impaye';

-- ── 3. recalc_client_debt ─────────────────────────────────────────────────────
